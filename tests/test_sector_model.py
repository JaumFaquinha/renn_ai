...
Cobre:
  ...
  - Setor sem campo 'delta_vs_best' é ignorado sem exception
...
def _make_lap(
    ...
    Se delta_high é fornecido, metade dos setores recebe delta_high e a outra
    metade recebe delta_low — útil para testar ordenação de scores.
    """
    sectors = []
...
class TestTrainInsufficient:
    ...
    def test_train_ignores_sectors_without_target(self) -> None:
        """Setores sem 'delta_vs_best' devem ser silenciosamente descartados."""
        # Cria 300 setores, mas sem o campo target
        sectors_no_target = [
            {k: v for k, v in _make_sector().items() if k != "delta_vs_best"}
            for _ in range(300)
        ]
...
# ---------------------------------------------------------------------------
# 4 — Ordenação de scores: setores com alta perda pontuam mais
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5 — Coerência predict() vs predict_batch()
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7 — Métricas de evaluate_model() (via scripts/train_model.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 8 — Robustez: setor com campos ausentes não lança exceção
# ---------------------------------------------------------------------------
